...
import emu.jectrex.config.AppConfig;
...
import emu.jectrex.ui.PagedScrollPane;
import emu.jectrex.ui.TextInputResponseHandler;
import emu.jectrex.ui.ViewportManager;

/**
 * The Home screen of the Jectrex emulator, i.e. the one that shows all of the 
...
public class HomeScreen extends InputAdapter implements Screen  {
   ...
   */
  private Properties screenShotProperties;
   ...
   * @param dialogHandler
   */
  public HomeScreen(Jectrex jectrex, DialogHandler dialogHandler) {
    ...
    //String appConfigJson = jectrex.getPreferences().getString("home_screen_app_list", DEFAULT_APP_CONFIG_JSON);
    jectrex.getPreferences().putString("home_screen_app_list", appConfigJson);
    ...
    screenShotProperties = new Properties();
    loadScreenShotProperties(Gdx.files.internal("data/jectrex_screens.store"));
    ...
    backgroundLandscape = new Texture("jpg/atmos_red_back_landscape.jpg");
    backgroundLandscape.setFilter(Texture.TextureFilter.Linear, Texture.TextureFilter.Linear);
    backgroundPortrait = new Texture("jpg/atmos_red_back_portrait.jpg");
  ...
  private void loadScreenShotProperties(FileHandle file) {
    InputStream in = null;
    try {
      in = new BufferedInputStream(file.read());
      screenShotProperties.loadFromXML(in);
    } catch (Throwable t) {
      Gdx.app.log("HomeScreen", "Failed to load screenshot Properties file.", t);
      t.printStackTrace();
    } finally {
  ...
  private void addAppButtonsToStage(Stage stage, AppConfig appConfig, int columns, int rows) {
    ...
    Texture titleTexture = new Texture("png/jectrex_title.png");
    ...
    if (keycode == Keys.BACK) {
      if (Gdx.app.getType().equals(ApplicationType.Android)) {
        dialogHandler.confirm("Do you really want to Exit?", new ConfirmResponseHandler() {
          public void yes() {
            // Pressing BACK from the home screen will leave Jectrex.
            Gdx.app.exit();
          }
          public void no() {
  ...
  public Button buildAppButton(AppConfigItem appConfigItem) {
    ...
    if ((appConfigItem.getIconPath() != null) && (!appConfigItem.getIconPath().equals(""))) {
      ...
      String screenshotData = screenShotProperties.getProperty(friendlyAppName, ""); 
      if (screenshotData == "") {
        screenshotData = jectrex.getScreenshotStore().getString(friendlyAppName, "");
      }
      if (screenshotData != "") {
        try {
      ...
      if ((appName != null) && (!appName.equals(""))) {
        ...
        if (appConfigItem != null) {
          if (longPressActor != null) {
            ...
            MachineScreen machineScreen = jectrex.getMachineScreen();
            machineScreen.initMachine(appConfigItem);
            jectrex.setScreen(machineScreen);
          }
        }
      } else {
        String startPath = jectrex.getPreferences().getString("open_app_start_path", null);
        dialogHandler.openFileDialog("", startPath, new OpenFileResponseHandler() {
          ...
          public void openFileResult(boolean success, final String filePath) {
            if (success && (filePath != null) && (!filePath.isEmpty())) {
              ...
              jectrex.getPreferences().putString("open_app_start_path", fileHandle.parent().path());
              jectrex.getPreferences().flush();
